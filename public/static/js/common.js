...
$(function() {
	//头部下拉菜单
	$(".sub-menu").parent().bind({
			mouseenter:function(){
			$(this).find(".sub-menu").css('min-width', $(this).width()).stop().slideDown();
		},
		mouseleave:function(){
	...
	//首页资讯上下滚动
	window.auto_scroll = function(obj) {
		$(obj).find("ul:first").animate({marginTop: "-50px"}, 500, function() {
			$(this).css({marginTop: "0px"}).find("li:first").appendTo(this);
		});
	}
	$(function() {
		setInterval('auto_scroll("#re_new")', 3000);
	});
...
// ---------  手机端开始 -------------
	...
	//左侧菜单栏
	$(".all_column").click(function(){
		var $menu = $(".brand_box");
		if ($menu.hasClass('brand_box_open') ) {
			$menu.removeClass('brand_box_open');
		}
		else {
			$menu.addClass('brand_box_open');
		}
...
function switchlang(lang) {
	...
	var basehref = baseurl.replace('http://'+window.location.host,'');
	var langs = ['zh','zh-tw', 'de', 'en', 'es', 'fa', 'fr', 'ja', 'tr','hk','ko','it','ru'];
	var sub_lan = basehref.split('/')[1];
	var sublanIndex = $.inArray(sub_lan,langs);
	var changelan = '';
	if(lang == 'zh'){changelan = '';}else{changelan = '/'+lang;}
	if(sublanIndex>0){
		href = basehref.replace('/'+langs[sublanIndex]+'/',changelan+'/');		
	}else{
		href = changelan+basehref;
	}
	window.location.href = href;
	return;
}

var $divs = $('ul.slides > li'), index = 0, len = $divs.length - 1;
$divs.find('.banner_img').css({
	'height' : '350px'
});
